...
const welcome = (req) =>
  new Promise((resolve) =>
    (async () => {
      ...
      await drawTableRow(doc, col, row, [1, full_name, courseName, start])
      ...
      await doc.font('Helvetica-Bold').text('Accommodation', col, doc.y + 10, {
        underline: true
      })
      ...
      if (parseInt(id_card, 10) > 0) {
        await doc.text('and ID card ', { continued: true })
      }

      if (certificateValidity > 0) {
        const word = toWord(certificateValidity)
        await doc.text(`valid for ${word} (${certificateValidity}) years`, {
          continued: true
        })
      }
      ...
      await doc.text('Management.', doc.x, doc.y + 30)

      await doc.text('Tolmann Allied Services Company Ltd', { oblique: true })
...
const writeHeader = (doc, col, row, textWidth) => {
  new Promise((resolve) =>
    (async () => {
      ...
      await doc
        .font('Helvetica')
        .text('COURSE JOINING INSTRUCTIONS', col, row, {
          align: 'left',
          width: textWidth
        })
...
const drawTableRow = async (doc, col, row, cols) =>
  new Promise((resolve) =>
    (async () => {
      ...
      await doc.rect(left, top, width, height).stroke()

      await doc.text(cols[0], left, top + 10, {
        align: 'center',
        width: width
      })
      ...
      await doc.text(cols[1], left, top + 10, {
        align: 'center',
        width: width
      })
      ...
      await doc.text(cols[2], left, top + 10, {
        align: 'center',
        width: width
      })
      ...
      await doc.text(cols[3], left, top + 10, {
        align: 'center',
        width: width
      })
      resolve()
